...
export const grave = Array(count)

for (let i = 0; i < count; i++) {
    grave[i] = new THREE.Mesh(graveGeo, graveMaterial)
    const angle = Math.random() * Math.PI * 2
    const radius = 3 + Math.random() * 4.5
    const x_Circle = (Math.sin(angle) * radius) + (Math.sin(angle) * radius)
    const z_Circle = (Math.cos(angle) * radius) + (Math.cos(angle) * radius)
...
export const updateGravesScaleY = (dataArray) => {
    ...
    const scaleYFactor = 0.5; //  adjust this factor based on how much you want the graves to scale
